class UsersController < ApplicationController
  def create
    ...
    if @user.save!
      ...
      json_response({:message=>"User created! please navigate to localhost:3000 and pass your new token as 'Authorization' in the header.", :username=>@user.username, :password=>@user.password, :token=>token.token_hash}, 201)
    else
